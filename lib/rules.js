...
export const DEFAULT_RULES = Object.freeze({
  minLength: 2,
  allowLoanwords: false, // 외래어
  allowCompound: false,  // 합성어·파생어 (표제어에 '-')
  allowDialect: false,   // 방언 + 비표준 표기(⇒규범 표기는 …)
  allowOld: false,       // 옛말
  allowNorth: false,     // 북한어(문화어)
  allowProper: false,    // 고유 명사 (인명·지명·책명·작품명)
});

/** 방 설정 UI 등에서 쓰는 규칙 옵션 설명 */
export const RULE_OPTIONS = Object.freeze([
  { key: 'allowDialect', label: '방언·비표준어', chipOn: '방언 ○', chipOff: '방언 ✕' },
  { key: 'allowOld', label: '옛말', chipOn: '옛말 ○', chipOff: '옛말 ✕' },
  { key: 'allowNorth', label: '북한어(문화어)', chipOn: '북한어 ○', chipOff: '북한어 ✕' },
  { key: 'allowLoanwords', label: '외래어', chipOn: '외래어 ○', chipOff: '외래어 ✕' },
  { key: 'allowCompound', label: '합성어·파생어', chipOn: '합성어 ○', chipOff: '합성어 ✕' },
  { key: 'allowProper', label: '고유 명사', chipOn: '고유명사 ○', chipOff: '고유명사 ✕' },
]);

/** 임의의 객체에서 규칙 옵션만 안전하게 골라낸다 (클라이언트 입력용) */
export function sanitizeRules(input = {}) {
  const out = { ...DEFAULT_RULES };
  for (const { key } of RULE_OPTIONS) if (input[key] != null) out[key] = Boolean(input[key]);
  const n = Number(input.minLength);
  if (Number.isInteger(n) && n >= 1 && n <= 10) out.minLength = n;
  return out;
}
...
const TYPE_ALLOW_KEY = { dialect: 'allowDialect', north: 'allowNorth', old: 'allowOld' };
...
function senseReason(sense, item, opts = DEFAULT_RULES) {
  const typeReason = NON_STANDARD_TYPES[sense.type];
  if (typeReason && !opts[TYPE_ALLOW_KEY[typeReason]]) return typeReason;
  ...
  if (!opts.allowProper && isProperNoun(sense)) return 'proper';
  if (!opts.allowDialect && isNonStandardDefinition(sense.definition)) return 'nonstandard';
  return null;
}
...
export function evaluateItem(rawItem, rules = DEFAULT_RULES) {
  ...
  for (const sense of item.senses) {
    let r = senseReason(sense, item, opts);
