...
// 검색 요청에 규칙 필터(searchFilters)를 붙여 명사·일반어·단어(·비외래어)만 받아 오고,
// 응답만으로 알 수 없는 부분(합성어 '-', 고유 명사 cat, 비표준 뜻풀이)은 passingEntries 로 다시 거른다.

import { passingEntries, searchFilters, DEFAULT_RULES } from './rules.js';
 ...
 */
export async function pickBotWord(client, { starts, used = new Set(), rules = DEFAULT_RULES, level = 'normal', rng = Math.random }) {
  const maxPages = { easy: 1, normal: 2, hard: 4 }[level] ?? 2;
  const candidates = new Map();
  const filters = searchFilters(rules);

  for (const start of starts) {
    const first = await client.search({ q: start, method: 'start', num: PAGE, start: 1, filters });
    collect(first.items, start, used, rules, candidates);

    const totalPages = Math.min(1000, Math.ceil(first.total / PAGE));
    if (totalPages > 1) {
      // 앞 페이지만 보면 늘 비슷한 단어가 나오므로, 무작위 페이지를 몇 장 더 본다.
      const pages = new Set();
      ...
      for (const p of pages) {
        try {
          const res = await client.search({ q: start, method: 'start', num: PAGE, start: p, filters });
