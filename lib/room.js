...
export const LIMITS = Object.freeze({
  turnSec: [5, 120], rounds: [1, 10], maxPlayers: [1, 8], quizCount: [3, 30], seatsPerClient: 4, nameLen: 12, roomNameLen: 24,
});
...
export class Room {
  ...
  constructor({ id, name, mode, settings = {}, hostClientId, deps }) {
    ...
    this.settings = sanitizeSettings(settings, {
      turnSec: 15, rounds: 3, maxPlayers: 4, quizCount: 10, level: 'normal', dueum: true, isPublic: true, rules: { ...DEFAULT_RULES },
    });
  ...
  scheduleBot(bot, token) {
    ...
    this.botTimer = setTimeout(async () => {
      ...
      if (!entry || giveUp) {
        if (this.mode.turnBased) this.endRound(this.players.indexOf(bot), `${bot.name}${iga(bot.name)} 이어갈 단어를 찾지 못했어요`);
        return;
      }
  ...
  async onTimeout(token) {
    ...
    if (this.mode.turnBased) {
      const p = this.players[this.turn];
      const name = p?.name ?? '?';
      this.endRound(this.turn, `${name}${iga(name)} 시간 안에 답하지 못했어요`);
    } else {
